...
# python main_mc.py --output_dir logs/DINO/R50_COCO_31 -c config/DINO/DINO_5scale.py --coco_path ../coco/ --eval --resume ./ckpts/checkpoint0031_5scale.pth --active_test_type ASE_mAP --result_json_path ./data/5_scale_31/val/mAP/results_10.json --test_sample_size 10 --seed 5 --start_idx 10 --options dn_scalar=100 embed_init_tgt=TRUE dn_label_coef=1.0 dn_bbox_coef=1.0 use_ema=False dn_box_noise_scale=1.0

test_type = "ASE_mAP"
json_path = "./data/5_scale_31/val/mAP/results_1.json"
basic_command = "CUDA_VISIBLE_DEVICES=1 python main_mc.py --output_dir logs/DINO/R50_COCO_temp_1 -c config/DINO/DINO_5scale.py --coco_path ../coco/ --eval --resume ./ckpts/checkpoint0031_5scale.pth --test_sample_size 1 --seed 5"
basic_command += " --active_test_type " + test_type
basic_command += " --result_json_path " + json_path
later_command = " --options dn_scalar=100 embed_init_tgt=TRUE dn_label_coef=1.0 dn_bbox_coef=1.0 use_ema=False dn_box_noise_scale=1.0 "

start_idxs = np.arange(0, 5001, 1)
for idx in start_idxs:
    command = basic_command + " --start_idx " + str(idx) + later_command + ">./results/R50_COCO_31_1.logs 2>./results/R50_COCO_31_1.err&"
    print(command)
    os.system(command)
    time.sleep(15)